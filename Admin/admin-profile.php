...
if (strlen($_SESSION['odmsaid']==0)) {
  ...
  {
    $A_ID=$_SESSION['odmsaid'];
    $A_Name=$_POST['adminname'];
  $A_PhoneNumber=$_POST['mobilenumber'];
  $A_Email=$_POST['email'];
  $sql="update admin set A_Name=:adminname,A_PhoneNumber=:mobilenumber,A_Email=:email where A_ID=:aid";
     $query = $dbh->prepare($sql);
     $query->bindParam(':adminname',$A_Name,PDO::PARAM_STR);
     $query->bindParam(':email',$A_Email,PDO::PARAM_STR);
     $query->bindParam(':mobilenumber',$A_PhoneNumber,PDO::PARAM_STR);
     $query->bindParam(':aid',$A_ID,PDO::PARAM_STR);
$query->execute();
...
<!doctype html>
 <html lang="en" class="no-focus"> <!--<![endif]-->
    <head>
 <title>Online BookVenue System</title>
...
{               ?>
                                    <form method="post">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-12">
                                                <input type="text" class="form-control" name="adminname" readonly value="Yash" required='true'>
                                            </div>
                                        </div>
                                        <div class="form-group row">
